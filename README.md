...
2. 사용자 입력
   ...
   - 입력값을 Ball 리스트로 변환
3. 힌트 출력
   - 사용자, 컴퓨터의 Ball 리스트를 비교하여 힌트를 가진 객체를 생성
   - 힌트 출력
4. 힌트에 따라 프로그램 진행
   1. 3스트라이크 일 때
      - 종료 여부 결정 (1 입력시 reset, 2입력 시 종료)
   2. 아닐 때
      - 2번 부터 다시 진행
...
- [x] Hint : 사용자에게 힌트를 알려주는 객체
  - [x] Map - key: BallStatus , value: 개수 (BallStatus : STRIKE, BALL, NOTHING)
  - [x] 기준이 되는 Ball 리스트를 가진다.
  - [x] Balls가 입력되면 기준이 되는 Balls와 비교해 Map에 힌트 저장
  - [x] Ball이 들어오면 해당 Ball이 기준이 되는 Balls와 비교하여 타입 반환
- [x] View : 사용자가 볼 수 있는 UI를 담당
  - [x] 입력 UI
  - [x] 힌트 UI
  - [x] 3스트라이크일 때 UI
- [x] Application : 컴퓨터, 사용자 입력에 숫자야구를 만들어준다, 게임 진행 흐름을 제어한다.
  - [x] 사용자와 컴퓨터를 비교하여 힌트 객체 반환
  - [x] 힌트 객체의 결과에 따라 게임 진행 흐름 제어
  - Ball 리스트 (숫자야구) 변환 (변환할 때 Set으로 중복 제거)
    - [x] 임의의 수 3개를 가진 Ball 리스트 변환
    - [x] 사용자 입력을 Ball 리스트로 변환
  - 사용자 입력 유효성 검사
    - [x] 입력값의 길이가 3인지 검사
    - [x] 입력값이 숫자로 구성되어있는지 검사
    - [x] play : 게임의 진행 여부 변수
## 🔍 진행방식
