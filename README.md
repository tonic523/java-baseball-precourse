...
---
### 📝 스토리보드
1. 게임이 실행되면 컴퓨터는 임의의 수 3자리의 Balls를 갖게 된다.
2. 사용자는 3자리의 수를 입력하여 사용자의 Balls를 갖는다.
   - 유효성 검사 : 3자리 수인지, 1~9의 숫자인지, 중복되는 숫자가 있는지
3. 컴퓨터의 Balls와 사용자의 Balls를 비교해 Hint를 얻는다.
   - Hint가 3 STRIKE 이면 게임 종료 (사용자가 게임을 진행할지 선택)
   - 아니라면 Hint를 사용자에게 출력 (2번 부터 다시 진행)
...
- [x] Application
  ...
  - [x] 사용자 입력이 숫자인지 검사
  - [x] 입력을 Balls로 바꿔준다.
  - [x] 임의의 수를 가진 Balls 생성

- [x] Ball
  - [x] 숫자를 갖는다.
  - [x] 숫자를 반환한다.
  - [x] 생성될 때 숫자가 1 ~9인지 검사
  - [x] 같은 객체끼리 숫자가 같은지 비교
...
- [x] Hint
  - [x] 2개의 Balls를 비교해 BALL, STRIKE, NOTHING의 개수를 저장
  - [x] BALL의 개수 반환
  - [x] STRIKE의 개수 반환
  - [x] NOTHING의 개수 반환
